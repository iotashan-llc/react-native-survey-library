...
/**
 * A2 consumer style-override surface (design ownership table: "A12
 ...
 * prop. Component wiring (accepting these props, threading them through)
 * is per-port (0.7 only defines the type + merge order).
 */
 ...
 * refines it, the consumer's explicit override always wins last. `flat`
 * + filters out nullish entries so callers can pass optional layers
 * directly.
 */
export function composeStyles<S extends RNStyle = RNStyle>(
  ...
  const groups: Array<S | S[] | undefined> = [
    recipeFragments,
    layers?.theme,
    layers?.override,
  ];
