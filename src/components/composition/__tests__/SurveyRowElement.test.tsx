...
describe('SurveyRowElement — dispatch', () => {
  ...
  it('falls back to UnsupportedQuestion on a dispatch miss (planned type "tagbox"), never throwing', () => {
    // 'tagbox' stays planned until M2 (task 2.6) — 'text' landed as
    // supported in task 1.10, so it can no longer model a dispatch miss.
    const { model, question } = firstQuestion({
      elements: [{ type: 'tagbox', name: 'q-text' }],
    });
    render(
      <SurveyRowElement
        element={question}
...
describe('SurveyRowElement — QuestionChrome at the dispatch boundary (M1 dispatcher contract)', () => {
  ...
  it('the unsupported fallback is wrapped in chrome too (title chrome renders around the fallback panel)', () => {
    const { model, question } = firstQuestion({
      elements: [{ type: 'tagbox', name: 'q-text', title: 'Text Title' }],
    });
    render(
      <SurveyRowElement
        element={question}
