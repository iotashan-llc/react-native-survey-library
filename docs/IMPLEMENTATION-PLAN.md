...
| A4 | **Own string-keyed factories** (`RNQuestionFactory`, `RNElementFactory`) mirroring upstream API shape; registration is centralized — components are side-effect-free, a descriptor-table-driven registrar (`register-all.ts`) is the single registration source (amended per 0.5 design review; upstream-style self-registration is tree-shake-fragile and defeats A10 lazy-require); **non-throwing unsupported-type fallback** with structured diagnostics. Must also cover SurveyJS runtime templates: `custom` + `composite` (ComponentCollection) get dedicated adapters (task 2.11); a **runtime-template coverage manifest** (classified model-type inventory + per-factory key inventories) is CI-checked alongside the export gate. Design: `docs/design/0.5-factories.md`. | Same seam upstream uses; `QuestionCustomModel.getTemplate()` returns `custom`/`composite`, not the registered type name. |
...
| A6 | **Theme = two-stage**: `theme-core` (pure ITheme → normalized tokens: rgba colors pass through, px parsed, `--sjs-base-unit` spacing fn, box-shadow strings parsed to objects, article typography; bounded fallbacks for unknown/invalid values) + `theme-rn` (tokens → StyleSheet factories; box-shadow arrays — offset/blur/spread/inset, multi-layer — direct-mapped on iOS + Android API ≥ 29, with a three-tier runtime fallback on Android API ≤ 28: inset layers dropped at API 28, single-`elevation` heuristic below). Golden tests against **all 40 theme objects from `survey-core`'s `themes/index.ts`** (predefined-themes.json only has 38). | Theme JSON in, native styles out, zero consumer config. |
