...
| A2 | Env shim (no-op `window.addEventListener/removeEventListener`, `ResizeObserver` stub) shipped THREE ways: (1) auto-applied by our main entry before any survey-core evaluation, (2) a **zero-core-import shim subpath** (`…/shim`) for consumers who import `survey-core` first (documented import-order contract), (3) exported `applySurveyCoreShims()`. All survey-core imports inside the lib go through `src/core/facade.ts`; ESLint enforces internally. Packaged-entry tests cover BOTH import orders (renderer-first and model-first) for CJS + ESM. | RN aliases `window===global`, tripping survey-core's SSR guards; `dragdrop/dom-adapter.ts` runs `window.addEventListener` at module evaluation. Consumer building `model` before importing us would crash without (2). |
...
| 0.3 | `CORE` survey-core facade + env shims + `…/shim` subpath + import-order contract; require-time load tests under RN-shaped globals for CJS+ESM × both import orders | M | A2 |
